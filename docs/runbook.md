...
| Failure | Action |
|---|---|
| `403` API | Client gửi `ngrok-skip-browser-warning` để bypass interstitial và báo `ngrok_codes` nếu vẫn bị gateway chặn. Nếu response là JSON từ Case API, xác nhận secret không có dấu nháy/prefix thừa và token đã được BTC kích hoạt. Client tự trim whitespace nhưng không retry 403. |
