...
def _create_ensemble_predictor(config: dict):
    """Build the self-consistency + precedent + thinking ensemble predictor, with the single-pass predictor as fallback."""
    from .ensemble import EnsembleOutcomePredictor, OutcomeConfig, PrecedentBank
    from .prediction import create_predictor

    prediction_cfg = config["prediction"]
    baseline = create_predictor(prediction_cfg)  # single-pass predictor, also the fallback
    outcome_cfg = OutcomeConfig(
        use_precedents=bool(prediction_cfg.get("use_precedents", False)),
        num_precedents=int(prediction_cfg.get("num_precedents", 3)),
        self_consistency=int(prediction_cfg.get("self_consistency", 5)),
        thinking=bool(prediction_cfg.get("thinking", False)),
    )
    bank = None
    if outcome_cfg.use_precedents:
        from sentence_transformers import SentenceTransformer

        from .data import load_precedent_cases

        embedder = SentenceTransformer(
            config["law_retrieval"]["embedding_model"],
            revision=config["law_retrieval"].get("embedding_revision"),
        )
        bank = PrecedentBank(
            embedder, load_precedent_cases(config["paths"]["public_test"])
        )
    return EnsembleOutcomePredictor(
        baseline.backend, outcome_cfg, precedent_bank=bank, fallback=baseline
    )
    ...
    try:
        ...
        if to_prepare:
            ...
            if mock:
                ...
                case_cfg = config["case_retrieval"]
                case_retriever = CaseEvidenceRetriever(
                    client,
                    max_queries=int(case_cfg["max_queries"]),
                    saturation_patience=int(case_cfg.get("saturation_patience", 4)),
                    max_network_calls=int(case_cfg.get("max_network_calls_per_case", 40)),
                )
        ...
        if pending_cases:
            if mock:
                predictor = OutcomePredictor(FixedBackend())
            elif config["prediction"].get("use_ensemble"):
                predictor = _create_ensemble_predictor(config)
            else:
