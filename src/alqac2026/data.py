...
def load_precedent_cases(path: str | Path) -> list[dict]:
    """Load the labelled public cases as a precedent memory (case-based reasoning).

    Returns {case_id, query, gold, reasoning}. This reads public gold fields
    (verdict_label, court_reasoning) as an inference-time retrieval memory of other
    cases — allowed by the competition, since the public set is the only labelled
    signal — and is gated behind the prediction.use_precedents config toggle. The
    target case's own gold is never used: prediction excludes the queried case
    from retrieval.
    """
    cases = []
    for item in _read_json(path):
        ...
        cases.append(
            {
                ...
                "gold": label,
                "reasoning": str(item.get("court_reasoning") or "")[:400],
            }
        )
    return cases
