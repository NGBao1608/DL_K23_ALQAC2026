...
def validate_submission(
    ...
    for index, item in enumerate(submission):
        ...
        if not isinstance(item["case_evidence"], list) or not all(
            ...
            # Official Case Content API returns segment IDs shaped like
            # "case_4101_seg_<hash>"; enforce only that each chunk belongs to
            # this case rather than a specific segment-token spelling.
            if any(
                not chunk.startswith(f"{case_id}_")
